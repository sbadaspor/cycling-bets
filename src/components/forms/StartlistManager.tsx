...
export default function StartlistManager({ provas }: Props) {
  ...
  // Carregar contagem de ciclistas já existentes para a prova selecionada
  useEffect(() => {
    if (!provaId) {
      setExistentes(0)
      return
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium">Prova</label>
        <select
          value={provaId}
          onChange={(e) => {
        ...
        {provaId && (
          <p className="text-xs text-neutral-400">
            Startlist atual: <strong>{existentes}</strong> ciclistas
            {existentes > 0 && (
              <button
                onClick={handleApagar}
                disabled={loading}
                className="ml-3 text-red-400 underline hover:text-red-300 disabled:opacity-50"
              >
                apagar startlist
              </button>
            )}
