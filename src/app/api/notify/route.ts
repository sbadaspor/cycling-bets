...
// ── Helpers VAPID (copiados do futebol-das-tercas — não modificar) ──
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Se user_ids fornecido → envia apenas para esses utilizadores
    // Se não → envia para todos os subscritores
    ...
    return NextResponse.json({ enviadas, falhadas, total: rows?.length ?? 0 })
  } catch (err: unknown) {
